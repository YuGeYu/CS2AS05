...
describe('about sources modal', () => {
  ...
  it('requires the second layer before exposing the upstream action and resets on reopen', async () => {
    ...
    expect(wrapper.text()).toContain('ianlucas/cs2-css-inventory-simulator')
    expect(wrapper.text()).toContain('XBribo/CS2-Bot-Vision')
    expect(wrapper.text()).toContain('mrdoob/three.js')
    expect(wrapper.text()).toContain('SteamDatabase/GameTracking-CS2')
    expect(wrapper.text()).toContain('DrEAmSs59/CS2-insight-agent')
    ...
    expect(openReference.mock.calls.map(call => call[0])).toEqual(['bot-improver', 'botvision', 'threejs', 'demotracer', 'demoparser', 'cs-demo-manager', 'gametracking-cs2', 'insight-agent', 'inventory-simulator'])
