...
pub fn calculate(
    ...
    let effective_deaths = deaths.max(1) as f64;
    let kill_component = clamp(
        (kills as f64 + assists as f64 / 5.0) / effective_deaths,
        0.0,
        3.0,
    );
    ...
    let rating = clamp(
        0.80 * kill_component
            + 0.10 * damage_component
            + 0.05 * survival_component
            + 0.05 * assist_component,
        0.0,
        3.0,
    );
    Some(DemoRating {
...
mod tests {
    ...
    fn fixed_vectors_match_simple_rating_v1() {
        ...
        assert!((average.kill_component - 2.4667).abs() < 0.0001);
        ...
        assert!((average.rating - 2.1748).abs() < 0.0001);
        ...
        assert_eq!(dominant.kill_component, 3.0);
        ...
        assert!((dominant.rating - 2.8485).abs() < 0.0001);
    }
