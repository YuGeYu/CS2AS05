...
public sealed class CS2BotLlmChatPlugin : BasePlugin, IPluginConfig<BotChatConfig>
{
    ...
    private void OnPlayerChat(CCSPlayerController player, string message, bool teamChat)
    {
        ...
        var selector = new BotReplySelector(personaRegistry, config.Global, _random);
        ...
        if (!selection.ShouldReply || selection.PersonaId is null || selection.Bot is null)
        {
            if (config.Global.Debug.LogSelection)
            {
                Logger.LogInformation("No bot reply for {PlayerName}: {Reason}", player.PlayerName, selection.Reason);
            }
        ...
        if (!config.Global.RateLimit.AllowConcurrentRequests
            && Interlocked.CompareExchange(ref _pendingRequests, 1, 0) != 0)
        {
            LogSelectionInfo("Skipping reply for {PlayerName}: LLM request already pending.", player.PlayerName);
            _memory.AddPlayerMessage(cleanEvent, config.Global.Memory);
            return;
        }
        ...
        _memory.MarkReplyForPlayer(GetPlayerKey(player), selection.PersonaId, selection.Bot.Slot, now);

        if (config.Global.Debug.LogSelection)
        {
            Logger.LogInformation(
                "Selected persona {PersonaId} for {PlayerName}: {Reason}",
                selection.PersonaId,
                player.PlayerName,
                selection.Reason);
        }

        _ = ReplyToChatAsync(target, cleanEvent, selection, config, _shutdown.Token);
    }
