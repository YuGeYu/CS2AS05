...
public sealed class OpenAiChatClient
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        var request = OpenAiChatRequestBuilder.Build(provider, messages);
        ...
        {
            throw new InvalidOperationException($"LLM API returned {(int)response.StatusCode}: {TrimForLog(responseBody)}");
        }

        return OpenAiChatResponseParser.ParseContent(responseBody);
    }
