...
public partial class NadeSystemPlugin : BasePlugin
{
    ...
    private void TryReplay(CCSPlayerController bot, GrenadeData g, List<CCSPlayerController> allControllers)
    {
        ...
        ThrowReservation? pacingReservation = null;
        if (_botNadesMode != "max")
        {
            int pacingTeamSize = allControllers.Count(p => p.IsValid && p.IsBot && (int)p.TeamNum == (int)bot.TeamNum);
            pacingReservation = _pacingPolicy.TryBegin(
                (uint)bot.Index, bot.TeamNum, gtype, ThrowReason.PlannedReplay,
                Server.CurrentTime, _freezeEndTime, Math.Max(pacingTeamSize, 1), _botNadesMode);
            if (pacingReservation == null) return;
        }
        ...
        if (pacingReservation != null)
            _pacingPolicy.Commit(pacingReservation, Server.CurrentTime);
