...
internal sealed class NadePacingPolicy
{
    ...
    private readonly record struct PacingProfile(
        int FlashLimit,
        int OtherGrenadeLimit,
        int TotalGrenadeLimit,
        float BotThrowGapSeconds,
        float TeamThrowGapSeconds,
        int OpeningThrowsPerBot,
        int OpeningTeamLimit,
        int OpeningSmokeLimit,
        int OpeningOffensiveLimit,
        int PlannedSmokeLimit);

    // This policy only layers CS2AS05 pacing on top of upstream behavior.
    // max deliberately has no profile: it must remain entirely upstream-controlled.
    private static PacingProfile ProfileFor(string mode) => mode switch
    {
        // Low intensity: preserve the upstream "more" behavior while preventing bursts.
        "more" => new(3, 2, 7, 2f, 0.25f, 2, 5, 2, 4, 0),
        // Medium intensity: the balanced CS2AS05 default pacing.
        "normal" => new(2, 1, 5, 5f, 0.5f, 1, 5, 1, 3, 1),
        // High intensity: layer stronger pacing on top of upstream "less" limits.
        "less" => new(1, 1, 2, 8f, 1f, 1, 3, 1, 2, 1),
        _ => throw new ArgumentOutOfRangeException(nameof(mode), mode, "Unsupported paced NadeSystem mode"),
    };
    ...
    {
        if (mode == "max")
            throw new InvalidOperationException("max must bypass CS2AS05 pacing entirely");
        ...
        int openingBudget = Math.Min(Math.Max(aliveBotCount, 0), profile.OpeningTeamLimit);
        if (!CanBegin(botIndex, teamNum, normalized, reason, now, freezeEndTime, aliveBotCount, mode)) return null;
        bool usesNormalPlannedSmokeCap = profile.PlannedSmokeLimit > 0 && reason == ThrowReason.PlannedReplay && normalized == "smoke";
        ...
        float now, float freezeEndTime, int aliveBotCount, string mode = "normal")
    {
        ...
        var profile = ProfileFor(mode);
        string? normalized = NormalizeGrenadeType(grenadeType);
        if (normalized == null || !CanReserveHardLimit(botIndex, normalized, profile)) return false;
        bool usesNormalPlannedSmokeCap = profile.PlannedSmokeLimit > 0 && reason == ThrowReason.PlannedReplay && normalized == "smoke";
        if (usesNormalPlannedSmokeCap
            && GetValue(_normalPlannedSmokeByTeam, teamNum) + GetValue(_pendingNormalPlannedSmokeByTeam, teamNum) >= profile.PlannedSmokeLimit)
            return false;
        bool isNonEmergency = reason is not ThrowReason.DefuseCover and not ThrowReason.ExtinguishFire;
        ...
        if (_lastBotThrowAt.TryGetValue(botIndex, out float lastBot) && now - lastBot < profile.BotThrowGapSeconds) return false;
        if (_lastTeamThrowAt.TryGetValue(teamNum, out float lastTeam) && now - lastTeam < profile.TeamThrowGapSeconds) return false;
        bool isOpening = freezeEndTime > 0f && now >= freezeEndTime && now - freezeEndTime < OpeningWindowSeconds;
        if (!isOpening) return true;
        int openingBudget = Math.Min(Math.Max(aliveBotCount, 0), profile.OpeningTeamLimit);
        if (GetValue(_openingByBot, botIndex) + GetValue(_pendingOpeningByBot, botIndex) >= profile.OpeningThrowsPerBot) return false;
        if (reason != ThrowReason.PlantCover
            && GetValue(_openingByTeam, teamNum) + GetValue(_pendingOpeningByTeam, teamNum) >= openingBudget) return false;
        return reason == ThrowReason.PlantCover || CanBeginOpeningType(teamNum, normalized, true, profile);
    }

    private bool CanBeginOpeningType(int teamNum, string grenadeType, bool isOpening, PacingProfile profile)
    {
        if (!isOpening) return true;
        if (grenadeType == "smoke")
            return GetValue(_openingSmokeByTeam, teamNum) + GetValue(_pendingOpeningSmokeByTeam, teamNum) < profile.OpeningSmokeLimit;
        return GetValue(_openingOffensiveByTeam, teamNum) + GetValue(_pendingOpeningOffensiveByTeam, teamNum) < profile.OpeningOffensiveLimit;
    }
    ...
    private bool CanReserveHardLimit(uint botIndex, string grenadeType, PacingProfile profile)
    {
        ...
        return total < profile.TotalGrenadeLimit
            && count < (grenadeType == "flash" ? profile.FlashLimit : profile.OtherGrenadeLimit);
    }
