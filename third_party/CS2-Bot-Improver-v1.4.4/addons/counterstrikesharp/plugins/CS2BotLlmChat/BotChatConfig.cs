...
public sealed class LlmProviderConfig
{
    ...
    public string BaseUrl { get; set; } = "https://api.600318.xyz";

    [JsonPropertyName("Model")]
    public string Model { get; set; } = "swe-2-high";

    [JsonPropertyName("FallbackModels")]
    public string[] FallbackModels { get; set; } = ["swe-2-high", "deepseek-v4-flash-0731", "glm-5.3-flash", "doubao", "lingbao", "脑力自算"];
    ...
    public void Normalize()
    {
        BaseUrl = string.IsNullOrWhiteSpace(BaseUrl) ? "https://api.600318.xyz" : BaseUrl.Trim();
        Model = string.IsNullOrWhiteSpace(Model) ? "swe-2-high" : Model.Trim();
        FallbackModels = FallbackModels.Where(model => !string.IsNullOrWhiteSpace(model)).Select(model => model.Trim()).Distinct(StringComparer.OrdinalIgnoreCase).ToArray();
        ApiKey = ApiKey.Trim();
...
public sealed class RateLimitConfig
{
    ...
    [JsonPropertyName("maxRepliesPerMatch")]
    public int MaxRepliesPerMatch { get; set; } = 10;
    ...
    public void Normalize()
    {
        ...
        MaxRepliesPerMatch = Math.Clamp(MaxRepliesPerMatch, 0, 1000);
...
public sealed class AntiLoopConfig
{
    ...
    public bool IgnoreBotMessages { get; set; } = false;
    ...
    public bool IgnoreMessagesFromKnownBotSlots { get; set; } = false;
