...
fn roster_key(player: &PlayerEndMetaData) -> String {
    match player
        .steamid
        .filter(|steamid| crate::second_pass::parser_settings::valid_steamid64(*steamid))
    {
        Some(steamid) => format!("steam:{steamid}"),
        None => format!(
            "bot:{}:{}",
            player.user_id.unwrap_or(-1),
            player.controller_id.unwrap_or(-1)
        ),
    }
}
#[derive(PartialEq)]
...
impl<'a> Parser<'a> {
    ...
    fn combine_outputs(&self, second_pass_outputs: &mut Vec<SecondPassOutput>, first_pass_output: FirstPassOutput) -> DemoOutput {
        ...
        if outputs.len() == 1 {
            ...
            let roster = {
                let mut by_key: std::collections::BTreeMap<String, PlayerEndMetaData> = std::collections::BTreeMap::new();
                for p in output.roster {
                    let key = roster_key(&p);
                    by_key.entry(key).and_modify(|current| {
                        if current.team_number.is_none() { current.team_number = p.team_number; }
                        if current.user_id.is_none() { current.user_id = p.user_id; }
                        if current.controller_id.is_none() { current.controller_id = p.controller_id; }
                    }).or_insert(p);
                }
        ...
        let mut roster_by_key: std::collections::BTreeMap<String, PlayerEndMetaData> = std::collections::BTreeMap::new();
        ...
        for output in outputs {
            ...
            for p in output.roster {
                roster_by_key.insert(roster_key(&p), p);
            }
        ...
        DemoOutput {
            ...
            roster: roster_by_key.into_values().collect(),
            game_events,
            skins,
            convars,
            df: all_dfs_combined,
