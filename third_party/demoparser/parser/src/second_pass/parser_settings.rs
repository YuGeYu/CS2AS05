...
pub fn valid_steamid64(steamid: u64) -> bool {
    steamid >= 76_561_197_960_265_728
}
...
impl<'a> SecondPassParser<'a> {
    pub fn create_output(self) -> SecondPassOutput {
        SecondPassOutput {
            ...
            roster: self.players.values().map(|p| PlayerEndMetaData {
                    steamid: p.steamid.filter(|steamid| valid_steamid64(*steamid)),
                    name: p.name.clone(),
                    team_number: p.team_num.map(|t| t as i32),
                    user_id: None,
                    controller_id: p.controller_entid,
                    is_bot: p.steamid.is_none_or(|steamid| !valid_steamid64(steamid)),
                }).chain(self.stringtable_players.values().map(|p| PlayerEndMetaData {
                    steamid: valid_steamid64(p.steamid).then_some(p.steamid),
