        ...
        for field in fields {
            ...
            if let Some(Variant::I32(u)) = field.data {
                ...
                let entity_id = match entity_id {
                    Some(eid) => eid,
                    None => {
                        if let Some(player) = self.find_user_by_userid(u) {
                            extra_fields.push(EventField {
                                name: prefix.to_owned() + "_name",
                                data: (!player.name.trim().is_empty()).then(|| Variant::String(player.name.clone())),
                            });
                            extra_fields.push(EventField {
                                name: prefix.to_owned() + "_steamid",
                                data: (player.steamid != 0).then_some(Variant::U64(player.steamid)),
                            });
                        } else {
                            // The raw userid remains available even when neither identity source resolves.
