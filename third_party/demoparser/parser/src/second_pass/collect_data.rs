...
impl<'a> SecondPassParser<'a> {
    pub fn collect_entities(&mut self) {
        if !self.prop_controller.event_with_velocity {
            // Event extraction and property extraction are independent outputs. When no
            // explicit tick filter is supplied, keep collecting properties even if the
            // caller also requested game events.
            if !self.wanted_ticks.is_empty() && !self.wanted_ticks.contains(&self.tick) {
                return;
            }
        }
