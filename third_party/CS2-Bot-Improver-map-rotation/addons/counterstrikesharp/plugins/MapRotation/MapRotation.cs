...
public sealed class MapRotationPlugin : BasePlugin
{
    ...
    {
        // The assistant installs this plugin specifically for automatic local-BOT rotation.
        // Keep the explicit console toggle for users who want to pause it temporarily.
        _enabled = LoadDefaultEnabled();
        ...
        Log($"Loaded. Default enabled={BoolToInt(_enabled)}, config={DefaultConfigRelativePath}. Rotation list: {FormatRotationList()}");
    ...
    private void EvaluateRoundEndAfterScoreUpdate(int evaluationSerial, int winner, int reason, string message)
    {
        ...
        ScheduleNextMap($"final-round detection: {matchState.DetectionReason}", delaySeconds: AutoChangeDelaySeconds);
    }
    ...
    private HookResult OnGameEnd(EventGameEnd @event, GameEventInfo info)
    {
        ...
        ScheduleNextMap($"EventGameEnd fallback winner={@event.Winner}", delaySeconds: AutoChangeDelaySeconds);
    ...
    private static bool LoadDefaultEnabled()
    {
        var path = Path.GetFullPath(Path.Combine(Server.GameDirectory, DefaultConfigRelativePath.Replace('/', Path.DirectorySeparatorChar)));
        try
        {
            if (!File.Exists(path))
            {
                Log($"Config missing: {path}; fallback enabled={BoolToInt(DefaultEnabled)}");
                return DefaultEnabled;
            }

            var bytes = File.ReadAllBytes(path);
            var sha256 = Convert.ToHexString(SHA256.HashData(bytes));
            var parsed = ParseDefaultConfig(bytes);
            if (!parsed.IsValid)
            {
                Log($"[MAP_ROTATION_CONFIG_INVALID] root={Path.GetFullPath(Server.GameDirectory)} path={path} sha256={sha256} reason={parsed.Reason}; fallback enabled={BoolToInt(DefaultEnabled)} source=fallback");
                return DefaultEnabled;
            }

            Log($"[MAP_ROTATION_CONFIG_READ] root={Path.GetFullPath(Server.GameDirectory)} path={path} sha256={sha256} enabled={BoolToInt(parsed.Enabled)} source=config loadSemantics=next-plugin-load");
            return parsed.Enabled;
        }
        catch (Exception exception)
        {
            Log($"Config read failed: {path}; fallback enabled={BoolToInt(DefaultEnabled)}; error={exception.Message}");
            return DefaultEnabled;
        }
    }

    private static ConfigParseResult ParseDefaultConfig(ReadOnlySpan<byte> bytes)
    {
        try
        {
            using var document = JsonDocument.Parse(new MemoryStream(bytes.ToArray(), writable: false));
            if (document.RootElement.ValueKind != JsonValueKind.Object)
            {
                return new ConfigParseResult(false, DefaultEnabled, "root-not-object");
            }
            if (!document.RootElement.TryGetProperty("enabled", out var enabled))
            {
                return new ConfigParseResult(false, DefaultEnabled, "enabled-missing");
            }
            if (enabled.ValueKind is not (JsonValueKind.True or JsonValueKind.False))
            {
                return new ConfigParseResult(false, DefaultEnabled, "enabled-not-boolean");
            }
            return new ConfigParseResult(true, enabled.GetBoolean(), "config");
        }
        catch (JsonException)
        {
            return new ConfigParseResult(false, DefaultEnabled, "invalid-json");
        }
    }
    ...
    private sealed record ConfigParseResult(bool IsValid, bool Enabled, string Reason);
