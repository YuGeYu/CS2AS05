...
internal static class Program
{
    ...
    private static void LessTotalCapAndReset()
    {
        var policy = new NadePacingPolicy();
        Commit(policy, Begin(policy, 1, 2, "flash", ThrowReason.PlannedReplay, 1f, 0f, 5, "less"), 1f);
        Commit(policy, Begin(policy, 1, 2, "flash", ThrowReason.DefuseCover, 1.1f, 0f, 5, "less"), 1.1f);
        Commit(policy, Begin(policy, 1, 2, "he", ThrowReason.Retaliation, 6.1f, 0f, 5, "less"), 6.1f);
        Commit(policy, Begin(policy, 1, 2, "smoke", ThrowReason.ExtinguishFire, 6.2f, 0f, 5, "less"), 6.2f);
        Reject(policy.TryBegin(1, 2, "molotov", ThrowReason.PlantCover, 11.2f, 0f, 5, "less"), "less fifth throw rejected across all entry reasons");
        policy.Reset();
        Accept(policy.TryBegin(1, 2, "molotov", ThrowReason.Retaliation, 2f, 0f, 5, "less"), "less total resets each round");
    }
