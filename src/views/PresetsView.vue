...
const nadeOptions = [{ value: 'max', label: '最多' }, { value: 'more', label: '较多' }, { value: 'normal', label: '正常' }, { value: 'off', label: '关闭' }] as const
...
<template>
  <section class="tool-view" aria-labelledby="presets-title">
    ...
    <section class="control-grid"><div class="control-group"><div><h2>Aim</h2><p>设置 Bot 的瞄准区域。</p></div><SegmentedControl :model-value="panel.snapshot?.presets.aim ?? null" :options="aimOptions" label="Aim" :disabled="blocked" @update:model-value="setAim" /></div><div class="control-group"><div><h2>Nades</h2><p>设置 Bot 使用投掷物的频率。</p></div><SegmentedControl :model-value="panel.snapshot?.presets.nades ?? null" :options="nadeOptions" label="Nades" :disabled="blocked" @update:model-value="setNades" /></div></section>
