...
import { clearAssistantData, getAssistantAccount, getAssistantPreferences, loginAssistant, logoutAssistant, openAccountRegister, openFaultIdeaPage, setAssistantAutostart, submitFaultReport, type AssistantAccount } from '@/services/tauri/support'
...
const botVisionEnabled = computed(() => store.environment?.botVisionEnabled ?? true)
const activeOperation = ref<'scan' | 'install' | 'panel' | 'uninstall' | 'preference' | 'clear' | 'account' | 'fault' | null>(null)
const operationMessage = ref('')
function focusBotVision() {
  const target = document.getElementById('bot-vision-setting')
  target?.scrollIntoView({
    behavior: matchMedia('(prefers-reduced-motion: reduce)').matches ? 'auto' : 'smooth',
    block: 'center',
  })
  target?.classList.remove('is-guided')
  requestAnimationFrame(() => target?.classList.add('is-guided'))
  window.setTimeout(() => target?.classList.remove('is-guided'), 2_000)
}
...
const diagnosis = computed(() => {
  if (!store.selectedRoot)
    return {
      tone: 'warning',
      title: '需要选择 CS2 目录',
      message: '先选择或扫描 Counter-Strike Global Offensive 目录。',
      action: '选择目录',
    }
  if (store.cs2Running && !store.writeUnlocked)
    return {
      tone: 'danger',
      title: '请先退出 CS2',
      message: '可在全局状态条确认已关闭后继续本地安装操作。',
      action: '等待退出',
    }
  if (store.environment?.baseEnvironmentReady)
    return {
      tone: 'ready',
      title: '环境可以更新',
      message: `已检测到定制插件包，可覆盖更新到 ${appConfig.appVersion}。`,
      action: '覆盖更新',
    }
  return {
    tone: 'info',
...
async function toggleBotVision(event: Event) {
  const enabled = (event.target as HTMLInputElement).checked
  activeOperation.value = 'preference'
  operationMessage.value = ''
  try {
    await store.setBotVision(enabled)
    operationMessage.value = enabled ? '体积烟已启用。' : '体积烟已关闭；下一场 BOT 对局生效。'
  } catch {
    ;(event.target as HTMLInputElement).checked = botVisionEnabled.value
    // Store exposes the actionable failure.
  } finally {
    activeOperation.value = null
  }
}
...
function focusAccount() {
  ...
  section?.scrollIntoView({ behavior: matchMedia('(prefers-reduced-motion: reduce)').matches ? 'auto' : 'smooth', block: 'end' })
  accountOpen.value = true
  nextTick(() => document.getElementById('account-username')?.focus())
}
...
async function submitAccountLogin() {
  ...
  try {
    ...
    window.dispatchEvent(new CustomEvent('cs2as:navigate', { detail: 'community' }))
  } catch (error) {
...
onMounted(async () => {
  window.addEventListener('cs2as:focus-bot-vision', focusBotVision)
  window.addEventListener('cs2as:focus-account', focusAccount)
  try {
...
onBeforeUnmount(() => { window.removeEventListener('cs2as:focus-bot-vision', focusBotVision); window.removeEventListener('cs2as:focus-account', focusAccount) })
</script>
    ...
    <section class="installer-shell" aria-labelledby="page-title">
      ...
      <section class="status-grid status-grid--diagnosis" aria-label="环境状态">
        <div class="status-item" :data-state="store.selectedRoot ? 'ready' : 'warn'">
          <span>CS2 目录</span><strong>{{ store.selectedRoot ? '已选择' : '未选择' }}</strong>
        </div>
        <div class="status-item" :data-state="processVisualState">
          <span>游戏进程</span><strong>{{ processLabel }}</strong>
        </div>
        <div class="status-item" :data-state="store.environment?.baseEnvironmentReady ? 'ready' : 'warn'">
          <span>定制插件包</span><strong>{{ packageState }}</strong>
        </div>
        <div class="status-item" :data-state="panel.snapshot?.ready ? 'ready' : 'warn'">
          <span>Panel</span><strong>{{ panel.snapshot?.ready ? '可用' : '待检查' }}</strong>
        </div>
      </section>
      ...
      <section class="install-section install-section--primary" aria-labelledby="install-title">
        ...
        <div class="install-action-stack">
          ...
          ><button class="primary-button install-cta" type="button" :disabled="installBlocked" @click="install">
            <ShieldCheck :size="20" /><span>{{ installLabel }}</span>
          </button>
          <p v-if="operationLabel" class="operation-state" aria-live="polite"><RefreshCw :size="14" class="spinning" />{{ operationLabel }}</p>
          <p v-else-if="operationMessage" class="operation-state operation-state--done" aria-live="polite"><ShieldCheck :size="14" />{{ operationMessage }}</p>
        </div>
      </section>
      ...
      <section class="assistant-settings" aria-labelledby="assistant-settings-title">
        ...
        <div id="bot-vision-setting" class="setting-row bot-vision-setting">
        ...
        <div class="setting-row">
          <span class="setting-icon" aria-hidden="true"><ShieldCheck :size="20" /></span>
          <div class="setting-copy">
            <strong>体积烟功能</strong>
            <span>体积烟可能增加部分电脑的卡顿；如果一场游戏出现卡顿，可以尝试取消勾选。</span>
          </div>
          <label class="toggle-switch" :aria-label="botVisionEnabled ? '关闭体积烟功能' : '开启体积烟功能'">
            <input type="checkbox" :checked="botVisionEnabled" :disabled="store.busy || !store.selectedRoot || !store.environment?.baseEnvironmentReady" @change="toggleBotVision" />
            <span aria-hidden="true"></span>
          </label>
        </div>
        <p class="appearance-hint">取消勾选会删除 <code>addons\metamod\BotVision.vdf</code>，不会删除 BotVision 的其他资源；重新勾选即可恢复。</p>
        <p v-if="maintenanceState" class="maintenance-state" aria-live="polite">
          {{ maintenanceState }}
        </p>
      </section>
      ...
      <section id="account-section" class="utility-section">
        <div class="utility-heading">
          <div>
            <p class="overline">维护与求助</p>
            <h2>问题留在这里，信息一次带齐</h2>
          </div>
        </div>
        <div class="maintenance-actions">
          ...
          <button class="secondary-button" type="button" @click="openAccountDialog">
            <LogIn :size="18" /><span>{{ account.loggedIn ? `已登录：${account.username}` : '登录官网账号' }}</span>
          </button>
    ...
    <Teleport to="body">
      ...
      <div v-if="clearConfirmOpen" class="modal-backdrop" role="presentation" @click.self="clearConfirmOpen = false">
        <section class="confirm-dialog" role="dialog" aria-modal="true" aria-labelledby="clear-data-title">
          ...
          <div class="dialog-actions">
            <button class="secondary-button" type="button" @click="clearConfirmOpen = false">取消</button><button class="danger-button" type="button" @click="confirmClearData"><Database :size="17" /><span>确认清除</span></button>
          </div>
        </section>
      </div>
      <div v-if="faultOpen" class="modal-backdrop" role="presentation" @click.self="faultOpen = false">
        <form class="fault-dialog" role="dialog" aria-modal="true" aria-labelledby="fault-title" @submit.prevent="submitFault">
          <header>
            <div>
              <p class="overline">故障工单</p>
              <h2 id="fault-title">告诉我们发生了什么</h2>
            </div>
            <button class="icon-button" type="button" aria-label="关闭故障提交" @click="faultOpen = false">
              <X :size="19" />
            </button>
          </header>
          ...
          <div class="fault-meta">
            <span><ShieldCheck :size="15" />诊断日志将随工单提交</span><span>{{ faultDetails.length }}/4000</span>
          </div>
          ...
          <div class="dialog-actions">
            <button class="secondary-button" type="button" @click="faultOpen = false">稍后再说</button
            ><button class="primary-button" type="submit" :disabled="faultDetails.trim().length < 10 || faultSubmitting">
              <Send :size="17" /><span>{{ faultSubmitting ? '正在提交…' : '提交故障单' }}</span>
            </button>
          </div>
        </form>
      </div>
      <div v-if="accountOpen" class="modal-backdrop" role="presentation" @click.self="accountOpen = false">
        <form class="confirm-dialog account-dialog" role="dialog" aria-modal="true" aria-labelledby="account-title" @submit.prevent="submitAccountLogin">
          <header>
            <div>
              <p class="overline">官网账号</p>
              <h2 id="account-title">登录 CS2AS</h2>
            </div>
            <button class="icon-button" type="button" aria-label="关闭账号登录" @click="accountOpen = false">
              <X :size="19" />
            </button>
          </header>
          ...
          <div class="dialog-actions">
            ...
            <button class="primary-button" type="submit" :disabled="accountBusy || !accountUsername.trim() || !accountPassword">
              {{ accountBusy ? '正在登录…' : '登录官网账号' }}
            </button>
            <button class="secondary-button" type="button" :disabled="accountBusy" @click="openAccountRegister">一键注册官网账号</button>
          </div>
        </form>
      </div>
    </Teleport>
