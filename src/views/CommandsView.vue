...
import { Check, ChevronDown, ChevronUp, Copy, Search, Terminal, Sparkles } from 'lucide-vue-next'
...
<template><section class="tool-view commands-view" aria-labelledby="commands-title"><header class="view-heading commands-hero"><div><p class="overline"><Terminal :size="14" /> CS2 控制台工具箱</p><h1 id="commands-title">命令</h1><p class="commands-lede">搜索、复制并快速应用本地 BOT 对局命令。</p></div><div class="commands-count"><Sparkles :size="16" /><strong>{{ entries.filter(item => item.copyable).length }}</strong><span>条可复制命令</span></div></header><div class="command-toolbar"><label class="command-search"><Search :size="18" /><input v-model="query" placeholder="搜索命令、队伍或参数" aria-label="搜索命令、队伍或参数" @input="activeIndex = 0" /><kbd>Ctrl K</kbd></label><div class="command-nav"><span>{{ matches.length ? activeIndex + 1 : 0 }} / {{ matches.length || 0 }}</span><button class="icon-button" title="上一个匹配" aria-label="上一个匹配" :disabled="!matches.length" @click="move(-1)"><ChevronUp :size="18" /></button><button class="icon-button" title="下一个匹配" aria-label="下一个匹配" :disabled="!matches.length" @click="move(1)"><ChevronDown :size="18" /></button></div></div><div ref="commandList" class="command-list"><template v-for="entry in entries" :key="entry.id"><h2 v-if="!entry.copyable">{{ entry.display }}</h2><button v-else type="button" :data-command-id="entry.id" :data-copied="copiedId === entry.id ? 'true' : undefined" :class="{ 'is-match': isMatch(entry.id), 'is-active': isActive(entry.id) }" @click="copy(entry.id, entry.copy)"><code>{{ entry.display }}</code><span class="command-copy-label">{{ copiedId === entry.id ? '已复制' : '复制' }}<Check v-if="copiedId === entry.id" :size="16" aria-hidden="true" /><Copy v-else :size="16" aria-hidden="true" /></span></button></template></div><p class="command-result" :data-tone="copyTone" :role="copyTone === 'error' ? 'alert' : undefined" aria-live="polite">{{ copyStatus }}</p></section></template>
