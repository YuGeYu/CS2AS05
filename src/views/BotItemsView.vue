...
<template><section class="tool-view" aria-labelledby="items-title"><header class="view-heading"><div><p class="overline">Bot Randomizer</p><h1 id="items-title">Bot 物品</h1></div></header><section class="toggle-list"><ToggleSwitch v-for="item in items" :key="item.key" :model-value="panel.snapshot?.botItems[item.key] ?? false" :label="item.label" :description="item.description" :disabled="!panel.snapshot?.ready || !!panel.mutationKey" @update:model-value="update(item.key, $event)" /></section><p v-if="cs2.cs2Running" class="warning-note">{{ cs2.writeUnlocked ? '已确认 CS2 关闭，修改已解锁；重新启动 CS2 后完整生效。' : '检测到 CS2 仍在运行，可在全局状态条确认已关闭后解锁。' }}</p><p v-if="panel.lastError" class="inline-error" role="alert">{{ panel.lastError }}</p></section></template>
