    ...
    <div v-if="open" class="appearance-drawer-layer" role="presentation" @click.self="emit('close')">
      <aside class="appearance-drawer" role="dialog" aria-modal="true" aria-labelledby="appearance-title">
        ...
        <div class="appearance-drawer__body">
          ...
          <section class="appearance-setting-group"><h3>启动体验</h3><label class="appearance-toggle-row"><span><strong>跳过开屏动画</strong><small>默认关闭；关闭后启动时直接进入助手主界面</small></span><input type="checkbox" :checked="preferences.skipIntro" @change="update({ skipIntro: ($event.target as HTMLInputElement).checked })" /><i aria-hidden="true"><b /></i></label><p class="appearance-hint">你可以随时在版本彩蛋中重新浏览水墨鸣谢长廊。</p></section>
