...
import { Copy, Megaphone, Minus, Palette, Square, X } from 'lucide-vue-next'
...
const emit = defineEmits<{ requestClose: []; openAppearance: [] }>()
...
function toggleMaximize() {
  ...
  let result: unknown
  try {
    result = appWindow.toggleMaximize()
  } catch {
    windowActionBusy.value = false
    return
  }
...
<template>
  <header class="app-titlebar" role="banner" @mousedown.left="startDragging" @dblclick="toggleMaximize">
    ...
    <div class="titlebar-brand">
      ...
      <span class="titlebar-brand-copy"
        ><span class="titlebar-name">CS2 人机增强助手</span><span class="titlebar-context" aria-live="polite">{{ currentContext }}</span></span
      >
    </div>
    ...
    <div class="titlebar-controls" aria-label="应用控制" @mousedown.stop @dblclick.stop>
      <div class="titlebar-control-group titlebar-control-group--app" aria-label="应用通知">
        <button class="titlebar-announcement-button" type="button" data-announcement-trigger :data-tone="announcementState.latest?.severity || 'idle'" :aria-expanded="announcementState.open" title="查看官网公告" aria-label="查看官网公告" @click="openAnnouncementCenter">
      ...
      <div class="titlebar-control-group titlebar-control-group--appearance" aria-label="外观设置">
        <button class="titlebar-button" type="button" title="打开外观工坊" aria-label="打开外观工坊" @click="emit('openAppearance')">
          <Palette :size="18" />
        </button>
      </div>
      <div class="titlebar-control-group titlebar-control-group--window" aria-label="窗口控制">
        <button class="titlebar-button" type="button" title="最小化" aria-label="最小化" @click="minimizeWindow">
          <Minus :size="18" />
        </button>
        ...
        <button class="titlebar-button titlebar-button--close" type="button" title="关闭" aria-label="关闭" @click="closeWindow">
          <X :size="18" />
        </button>
      </div>
    </div>
