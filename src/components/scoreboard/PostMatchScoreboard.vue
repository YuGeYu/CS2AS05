...
const groups = computed(() => [
  ...
  { key: 'unknown', label: '未识别阵营', players: props.report.players.filter(player => ![2, 3].includes(player.teamNumber ?? -1) && !['CT', 'T'].includes(player.team ?? '')) },
...
<template>
  <section class="post-scoreboard">
    <div class="match-band"><div><span>地图</span><strong>{{ report.summary.mapName || '--' }}</strong></div><div><span>比分</span><strong>{{ report.summary.teamAScore == null || report.summary.teamBScore == null ? '-- / 未验证' : `${report.summary.teamAScore} : ${report.summary.teamBScore}` }}</strong></div><div><span>报告回合</span><strong>{{ report.summary.totalRounds }}</strong></div><div class="match-file"><span>Demo</span><strong :title="report.summary.fileName">{{ report.summary.fileName }}</strong></div></div>
    <div class="honors"><div v-for="honor in honors" :key="honor.label"><span>{{ honor.label }}</span><strong>{{ honor.player?.name || '--' }}</strong></div></div>
    <div class="scoreboard-table-wrap"><table><thead><tr><th>玩家</th><th>K-D-A</th><th>ADR</th><th>HS%</th><th>简易 Rating</th></tr></thead><tbody v-for="group in groups" :key="group.key"><tr class="team-row" :data-team="group.key"><th colspan="5">{{ group.label }} · {{ group.players.length }} 人</th></tr><template v-for="player in group.players" :key="player.key"><tr class="player-row" tabindex="0" @click="expanded = expanded === player.key ? null : player.key" @keydown.enter="expanded = expanded === player.key ? null : player.key"><td :title="player.name || undefined">{{ player.name || '--' }} <small v-if="player.isBot">BOT</small></td><td>{{ value(player.kills) }}-{{ value(player.deaths) }}-{{ value(player.assists) }}</td><td>{{ value(player.adr, 1) }}</td><td>{{ percent(player.headshotPercent) }}</td><td><span class="rating" :data-rating="ratingClass(player)">{{ player.rating ? player.rating.rating.toFixed(2) : '--' }} {{ ratingLevel(player) }}</span></td></tr><tr v-if="expanded === player.key" class="detail-row"><td colspan="5"><div><span>身份 {{ player.steamId || `BOT / ${player.userId ?? '--'}` }}</span><span>报告回合 {{ value(player.roundsPlayed) }}</span><span>模型 {{ player.rating?.modelVersion || '--' }}</span></div><p>KAST、交易、Swing 与经济指标需要逐回合同步状态，本版本暂不提供。</p><p v-if="!player.rating">{{ report.dataQuality.ratingWarnings?.join('；') || '该玩家缺少 K/D/A、伤害或报告回合。' }}</p></td></tr></template></tbody></table></div>
    <footer><span>解析 {{ report.dataQuality.scoreboardStatus }} · Rating {{ report.dataQuality.ratingStatus }}</span><span>简易 Rating · simple-rating-v1 · 非 HLTV 官方模型</span></footer>
  </section>
</template>
